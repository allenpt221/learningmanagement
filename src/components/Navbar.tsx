        ...
        <div className='flex space-x-5 font-medium'>
          ...
          {profile?.success ? (
            <div className='flex items-center'>
              <p className='font-normal text-sm'>{profile.user?.username}</p>
              <img src={profile.user?.image} alt="error fetching profile" className='w-10 h-10' />
            </div>
          ) : (
            <div>
              <Link href='/login' className='border px-4 py-1 rounded-full'>
                Log In
              </Link>
